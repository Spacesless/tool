  ...
  <ToolLayout>
    <section class="section">
      <div class="section-header">
        <el-input v-model="keyword" placeholder="请输入名称或区号" clearable />
      </div>

      <el-table :data="filterData" border :height="600">
        <el-table-column prop="name" label="名称" />
        <el-table-column prop="englishName" label="英文名称" />
        <el-table-column prop="countryCode" label="地区编码" />
        <el-table-column prop="phoneCode" label="区号" />
      </el-table>
...
watch(() => keyword.value, () => {
  if (keyword.value) {
    tableData.value = list.filter(item => item.name.includes(keyword.value) || item.phoneCode.includes(keyword.value))
  } else {
